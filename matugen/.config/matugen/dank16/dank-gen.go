// Almost a copy paste of AvengeMedia/DankMaterialShell dank16 generator
// https://github.com/AvengeMedia/DankMaterialShell/blob/master/core/internal/dank16/dank16.go
package main
...
func main() {
	if len(os.Args) < 3 {
		fmt.Println("Usage: dank-gen <dark_primary_hex> <light_primary_hex> [background_hex]")
		os.Exit(1)
	}
	...
	if !strings.HasPrefix(darkHex, "#") || !strings.HasPrefix(lightHex, "#") {
		fmt.Fprintf(os.Stderr, "Error: Invalid color format. Expected hex (e.g., #ff0000), got dark: '%s', light: '%s'\n", darkHex, lightHex)
		os.Exit(1)
	}
	...
	palette := GenerateVariantPalette(opts)
